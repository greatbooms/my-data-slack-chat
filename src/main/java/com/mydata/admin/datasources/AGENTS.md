...
- `AdminIngestionJobService`: 활성 데이터소스의 job 집계와 실패 item keyset pagination을 처리합니다.
...
- `AdminIngestionJobItemPayload`, `AdminIngestionJobItemPagePayload`: item 상태와 cursor page 응답 모델입니다.
...
- job과 item 조회는 활성 워크스페이스/데이터소스 범위에서 fail-closed로 처리합니다.
- job item count는 목록 단위 집계 query로 가져와 N+1 query를 만들지 않습니다.
